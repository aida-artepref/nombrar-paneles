...
// ---- CSS2D Renderer (para etiquetas HTML) ----
...
world.renderer.three.domElement.parentElement?.appendChild(
  labelRenderer.domElement
);

// sincronizar tamaño al hacer resize
...
// renderizar etiquetas junto al mundo
world.renderer.three.setAnimationLoop(() => {
  world.renderer.three.render(world.scene.three, world.camera.three);
  labelRenderer.render(world.scene.three, world.camera.three);
});
...
// ✅ NO dejes que autoSetWasm toque tu configuración
ifcLoader.settings.autoSetWasm = false;

// ✅ fija wasm local
ifcLoader.settings.wasm = {
  path: `${import.meta.env.BASE_URL}web-ifc/`,
  absolute: true,
  logLevel: ifcLoader.settings.wasm.logLevel, // conserva el logLevel actual
};
...
// ✅ Vite-friendly worker URL + await (top-level await ya lo tienes)
await fragments.init(
  new URL("@thatopen/fragments/dist/Worker/worker.mjs", import.meta.url).href
);
...
fragments.list.onItemSet.add(async ({ value: model }) => {
  ...
  // 👇 1) Calcula la cota más baja del modelo (en coordenadas del mundo)
  model.object.updateMatrixWorld(true);
  const box = new THREE.Box3().setFromObject(model.object);
  const minY = box.min.y;

  // 👇 2) Coloca la rejilla en esa cota
  // Según versión, el objeto interno puede variar. Estas 3 líneas cubren casi todos los casos:
  (worldGrid as any).three?.position && ((worldGrid as any).three.position.y = minY);
  (worldGrid as any).mesh?.position && ((worldGrid as any).mesh.position.y = minY);
  (worldGrid as any).position && ((worldGrid as any).position.y = minY);
